...
class _WelcomePage(QWidget):
    ...
    def _build(self, name: str, desc: str):
        v = QVBoxLayout(self)
        v.setContentsMargins(40, 24, 40, 20)
        v.setSpacing(0)
        v.addStretch(1)
        ...
        # Contenedor central: Columna izquierda (Mensaje + Botón) y Columna derecha (QR)
        content_h = QHBoxLayout()
        content_h.setSpacing(36)
        content_h.setAlignment(Qt.AlignmentFlag.AlignCenter)

        # Columna izquierda: Mensaje de dedicatoria + Botón ENTRAR
        ...
        left_col.addWidget(msg)

        self.btn_enter = QPushButton("▶   ENTRAR")
        self.btn_enter.setObjectName("btn_enter")
        self.btn_enter.setFixedSize(190, 44)
        left_col.addWidget(self.btn_enter, alignment=Qt.AlignmentFlag.AlignCenter)

        content_h.addLayout(left_col)

        # Columna derecha: Tarjeta con Código QR + Texto indicador
        right_col = QVBoxLayout()
        right_col.setSpacing(8)
        right_col.setAlignment(Qt.AlignmentFlag.AlignCenter)

        qr_card = QFrame()
        qr_card.setFixedSize(142, 142)
        ...
        qr_card_layout.setContentsMargins(6, 6, 6, 6)

        qr_img_lbl = QLabel()
        qr_path = os.path.join(ASSETS_DIR, "playlist_qr.png")
        if os.path.exists(qr_path):
            px = QPixmap(qr_path).scaled(128, 128, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
            qr_img_lbl.setPixmap(px)
        qr_img_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        qr_img_lbl.setStyleSheet("background: transparent; border: none;")
        qr_card_layout.addWidget(qr_img_lbl)

        right_col.addWidget(qr_card, alignment=Qt.AlignmentFlag.AlignCenter)

        qr_caption = QLabel("Escanea la playlist 📱")
        qr_caption.setStyleSheet(f"color: {C_TEXT2}; font-size: 11px; background: transparent; font-weight: bold;")
        qr_caption.setAlignment(Qt.AlignmentFlag.AlignCenter)
        right_col.addWidget(qr_caption, alignment=Qt.AlignmentFlag.AlignCenter)

        content_h.addLayout(right_col)

        v.addLayout(content_h)
