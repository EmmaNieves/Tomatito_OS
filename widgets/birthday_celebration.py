...
class BirthdayCelebrationOverlay(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # "Wooow" (arriba a la izquierda, amarillo)
        painter.save()
        painter.translate(w * 0.14, h * 0.16)
        painter.rotate(-8)
        painter.setPen(QColor("#FFD93D"))
        painter.drawText(0, 0, "Wooow")
        painter.restore()
        
        # "¡Qué talento!" (arriba a la derecha, coral festivo)
        painter.save()
        painter.translate(w * 0.70, h * 0.16)
        painter.rotate(8)
        painter.setPen(QColor("#FF6B6B"))
        painter.drawText(0, 0, "¡Qué talento!")
        painter.restore()
        
        # "¡Disfruta tu día!" (abajo a la derecha, cian alegre)
        painter.save()
        painter.translate(w * 0.74, h * 0.72)
        painter.rotate(-5)
        painter.setPen(QColor("#6BCBEF"))
        painter.drawText(0, 0, "¡Disfruta tu día!")
        painter.restore()
